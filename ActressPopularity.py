...
#filtering out the actresss alone.
actresss = df_prin.filter(col("category") == "actress")
result1 = actresss.join(df_movies_final, df_movies_final['tconst'] == actresss['tconst'],'inner').drop(df_movies_final['tconst']) # principle joined with popularity index
...
actress_final = df_names.join(result2, result2['nconst'] == df_names['nconst'],'inner').drop(result2['nconst'])
actress_final = actress_final.join(result3, result3['nconst'] == actress_final['nconst'],'inner').drop(actress_final['nconst'])

actress_concat = actress_final.withColumn("aggregated_genres", concat_ws(", ", col("aggregated_genres")))
actress_concat.coalesce(1).write.csv("/user/s3176649/Results/actressPopularity", mode='overwrite', header=True)